...
class BTN:
    def __init__(
        ...
        (self.p_bonds, self.p_nodes, self.q_bonds, self.q_nodes, self.sigma) = (
            builder.build_model()
        )
    ...
    def _concat_batch_results(self, batch_results: List):
        ...
        elif hasattr(first_data, "__array__"):  # numpy-like
            import numpy as np
    ...
    def _trim_bond_by_variance(
        self, bond_tag, threshold=1e-10, relative=False, verbose=False
    ):
        _, nodes = self.count_trainable_nodes_on_bond(bond_tag)
        if not nodes:
            return 0
    ...
    def trim_bonds_by_variance(self, threshold=1e-10, relative=False, verbose=False):
        ...
        if total_trimmed > 0:
            ...
            if verbose:
                print(
                    f"Delta ELBO from variance trimming: {elbo_after - elbo_before:.4f}"
                )
    ...
    def _trim_bond_by_gamma(self, bond_tag, threshold=0.1, verbose=False):
        ...
        if verbose:
            print(
                f"  {bond_tag}: γ mean={gamma.mean():.4f}, min={gamma.min():.4f}, max={gamma.max():.4f}"
            )
    ...
    def predict_mean_var(self, data_stream=None):
        ...
        Predictive model (Gaussian):
            mean_i = forward(mu)_i
            var_i  = forward(sigma)_i        (epistemic, from weight posterior q)
                   + 1 / E[tau]              (aleatoric, observation noise)
        ...
        # Per-point predictive mean: keep batch dim, sum out output dims.
        mean = self.forward(self.mu, data_stream.data_mu,
                            sum_over_batch=False, sum_over_output=True)
        # Per-point epistemic variance from the covariance network.
        epi = self.forward(self.sigma, data_stream.data_sigma,
                           sum_over_batch=False, sum_over_output=True)

        mean = self._to_torch(mean).reshape(-1).to(torch.float64)
        epi = self._to_torch(epi).reshape(-1).to(torch.float64).clamp_min(0.0)

        tau_mean = self.q_tau.mean()
        tau_mean = tau_mean.data if isinstance(tau_mean, qt.Tensor) else tau_mean
        tau_mean = float(tau_mean.item() if torch.is_tensor(tau_mean) else tau_mean)
        aleatoric = torch.full_like(mean, 1.0 / max(tau_mean, 1e-12))
