...
class TNALS:
    def __init__(
        ...
        (self.p_bonds, self.p_nodes, self.q_bonds, self.q_nodes, self._sigma_template) = builder.build_model()
        self.sigma = None
    ...
    def compute_elbo(self, verbose=False, print_components=False, relative=True):
        return 0.0
    ...
    def update_bond(self, bond_tag, concentration=None, rate=None):
        """Update bond only if new values are explicitly provided."""
        if concentration is not None or rate is not None:
    ...
    def compute_precision_node(
        ...
        precision = tau_expectation * mu_outer_env + theta
        return precision
    ...
    def update_mu_node(self, node_tag):
        ...
        precision = self.compute_precision_node(node_tag)
        sigma_node = self.invert_ordered_tensor(precision, block_variational_ind, tag=node_tag)
        for i in block_output_ind:
            sigma_node.new_ind(i, self.mu.ind_size(i))
        ...
        relabel_map = {}
        for ind in mu_idx:
            if ind not in self.output_dimensions:
                relabel_map[ind] = ind + "_prime"
        rhs = rhs.reindex(relabel_map)
        rhs = rhs * self.q_tau.mean()
        tn = rhs & sigma_node
    ...
    def _trim_bond(self, bond_tag, verbose=False):
        """
        Trim bond dimensions based on cumulative relevance threshold.

        Keeps dimensions contributing to `threshold` fraction of total relevance.

        Since q expectations represent precision (not variance), we use 1/E[lambda]
        as the relevance measure - lower precision means higher variance means more relevant.
        """
        ...
        if len(indices_list) == len(weights):
            return 0
        ...
        for node in nodes:
            ...
            self.fancy_isel(self.mu[node_tag], {f"{bond_tag}": indices_list})

            self.fancy_isel(
                self.sigma[node_tag],
                {f"{bond_tag}": indices_list, f"{bond_tag}_prime": indices_list},
            )
        ...
        self._trim_input_data(bond_tag, indices_list)

        if verbose:
            print(f"Bond {bond_tag}: kept {len(indices_list)}/{len(weights)} dimensions")

        return len(weights) - len(indices_list)
    ...
    def _prepare_soft_trim_bond(self, bond_tag, high_precision=1e8, verbose=False):
        ...
        cutoff_idx = torch.searchsorted(normalized_cumsum, threshold).item()
        indices_to_keep = sorted_indices[: cutoff_idx + 1].tolist()
    ...
    def _get_bond_variances(self, node_tag, bond_tag):
        sigma = self.sigma[node_tag].copy()
        mu_inds = list(self.mu[node_tag].inds)

        variational_inds = [i for i in mu_inds if i not in self.output_dimensions]
        col_inds = sorted(variational_inds)
        row_inds = [f"{i}_prime" for i in col_inds]

        cov_matrix = sigma.to_dense(col_inds, row_inds)
        ...
        bond_only_inds = [i for i in col_inds if i in self.q_bonds]
        dims = [self.mu.ind_size(i) for i in bond_only_inds]

        if bond_tag not in bond_only_inds:
            return torch.zeros(self.mu.ind_size(bond_tag))

        other_dims = [self.mu.ind_size(i) for i in col_inds if i not in self.q_bonds]
        full_dims = dims + other_dims

        if len(full_dims) > 0 and diag.numel() == torch.prod(torch.tensor(full_dims)).item():
            diag_tensor = diag.reshape(*full_dims)
        else:
            diag_tensor = diag.reshape(*[self.mu.ind_size(i) for i in col_inds])

        bond_axis = col_inds.index(bond_tag)
        axes_to_mean = [i for i in range(len(col_inds)) if i != bond_axis]

        if axes_to_mean:
            bond_variance = diag_tensor.mean(dim=axes_to_mean)
        else:
            bond_variance = diag_tensor

        return bond_variance
    ...
    def _trim_bond_by_variance(self, bond_tag, threshold=1e-10, relative=False, verbose=False):
        ...
        if len(indices_to_keep) == 0:
            ...
            if verbose:
                print(f"Bond {bond_tag}: all below threshold, keeping max")
        ...
        if verbose:
            print(
                f"Bond {bond_tag}: kept {len(indices_to_keep)}/{len(total_variance)} (threshold: {effective_threshold:.2e})"
            )
