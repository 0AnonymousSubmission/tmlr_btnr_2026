...
def load_dataset(
    ...
    if csv_path is not None:
        ...
        uci_dataset_map = {
            name: (dataset_id, task) for name, dataset_id, task in uci_datasets
        }
        ...
        if _task != "regression":
            raise ValueError(
                f"Dataset '{dataset_name}' is a {_task} dataset. BTN only supports regression."
            )
...
def append_bias(data):
    ...
    Returns:
        New data dict with bias column appended (does NOT mutate original)
    """
    result = {}
...
def expand_features_polynomial(data, degree: int, device: str = "cpu"):
    ...
    for key in ["X_train", "X_val", "X_test"]:
        ...
        X_expanded = torch.zeros(
            n_samples, n_features, phys_dim, dtype=X.dtype, device=device
        )
...
def load_polynomial_dataset(
    ...
    if n_terms is not None and n_terms < len(terms):
        ...
        linear_terms = [
            tuple([1 if j == i else 0 for j in range(n_features)])
    ...
    if coefficients is not None:
        if len(coefficients) != n_total_terms:
            raise ValueError(
                f"Expected {n_total_terms} coefficients, got {len(coefficients)}"
            )
...
def _generate_polynomial_terms(
    n_features: int, degree: int, interaction_only: bool = False
):
    """
...
def load_featurewise_polynomial_dataset(
    ...
    raw_features = torch.randn(
        n_samples, n_features, dtype=torch.float64, device=device
    )
    ...
    if target_coefficients is not None:
        if len(target_coefficients) != n_terms:
            raise ValueError(
                f"Expected {n_terms} coefficients, got {len(target_coefficients)}"
            )
...
def load_uci_featurewise(
    dataset_name: str, degree: int = 2, device: str = "cpu", cap: int = 20
):
    """
    ...
    def expand_features(X):
        n_samples = X.shape[0]
        X_expanded = torch.zeros(
            n_samples, n_features, phys_dim, dtype=X.dtype, device=X.device
        )
        for p in range(phys_dim):
            X_expanded[:, :, p] = X**p
        return X_expanded
